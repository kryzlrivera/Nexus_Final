...
export default function PostCard({ post }: PostCardProps) {
  ...
  return (
    <div className="card animate-fade-in" style={{ marginBottom: '1.5rem', overflow: 'hidden' }}>
      ...
      {post.photoUrl ? (
        <img src={post.photoUrl} alt="Post content" style={{ width: '100%', aspectRatio: '1 / 1', objectFit: 'cover', display: 'block' }} />
      ) : (
      ...
      <div style={{ padding: '0.75rem 1rem 0.25rem 1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button onClick={() => toggleLike(post.id)} style={{ color: isLiked ? 'var(--error)' : 'var(--text-main)', padding: 0 }}>
