        ...
        <div className="profile-header-content">
          <div style={{ flexShrink: 0 }}>
            <div className="story-circle profile-avatar" style={{ width: '80px', height: '80px' }}>
              <img src={profileUser.avatar} alt={profileUser.name} className="story-circle-inner" />
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '2rem' }}>
          {isSelf ? (
            <>
              ...
              <button className="btn" style={{ flex: 1, backgroundColor: 'var(--border-color)', color: 'var(--text-main)', padding: '0.4rem', fontWeight: 600 }}>
                Share profile
              </button>
            </>
          ) : (
            <>
              {isFriend ? (
                <button onClick={() => removeFriend(username)} className="btn" style={{ flex: 1, backgroundColor: 'var(--border-color)', color: 'var(--text-main)', padding: '0.4rem', fontWeight: 600 }}>
                  Following
                </button>
              ) : (
                <button onClick={() => addFriend(username)} className="btn btn-primary" style={{ flex: 1, padding: '0.4rem', fontWeight: 600 }}>
                  Follow
                </button>
              )}
              <button className="btn" style={{ flex: 1, backgroundColor: 'var(--border-color)', color: 'var(--text-main)', padding: '0.4rem', fontWeight: 600 }}>
